...
nextpnr uses mruby as an embedded scripting language to enable customisation of the FPGA CAD flow.

If you are interested in using Ruby to describe the FPGA rather than customise or explore the flow, see the [nextpnr Generic Architecture](generic.md) documentation.

## Running Ruby Scripts

Ruby scripts can be run at any point in the flow. The following command line arguments are used to run a script:

 - **--pre-pack script.rb**: after reading the JSON, before packing
 - **--pre-place script.rb**: after packing, before placement
 - **--pre-route script.rb**: after placement, before routing
 - **--post-route script.rb**: after routing, before bitstream generation
...
Internally, nextpnr uses a fast indexed string pool called `IdString`. This is automatically and transparently converted to/from Ruby strings by the mruby binding layer. 

Likewise, the architecture identifiers `BelId`, `WireId` and `PipId` are also translated to/from Ruby strings containing the object name automatically.

To query the FPGA architecture, use the functions described in the [Architecture API documentation](archapi.md). Ranges can be iterated over in the same way as any other Ruby enumerable.

## Netlist Access

### Accessing nets

There is a hash `$ctx.nets` that provides access to all of the nets in a design by name. Each net item has the following fields:
 ...
 - `attrs`: a read-only hash of attributes on the net
...
There is a hash `$ctx.cells` that provides access to all of the cells in a design by name. Each cell item has the following fields:

 - `name`: name of the cell
 - `type`: type of the cell
 - `ports`: a read-only hash of ports on the cell. Each port has the following fields:
   - `name`: name of the port
   - `net`: reference to the connected net, or `nil` if disconnected
   - `type`: direction of the port 
 - `params`: a read-only hash of parameters on the cell
 - `attrs`: a read-only hash of attributes on the cell
